...
func TestMessageJSON(t *testing.T) {
	...
	MessageJSONTests := []struct {
		...
		},
		{
			...
			want: `{"type":8,"info":"Selene swapped a 'Q' for ['A','B','C'].","game":{"board":{"tiles":[{"id":3,"ch":"A"},{"id":1,"ch":"B"},{"id":7,"ch":"C"}]}}}`,
		},
		{
			m:    Message{Type: 9, Game: &game.Info{Board: newBoard(nil, []tile.Position{{Tile: tile.Tile{ID: 8, Ch: 'R'}, X: 4, Y: 46}}, nil)}},
			want: `{"type":9,"game":{"board":{"tilePositions":[{"t":{"id":8,"ch":"R"},"x":4,"y":46}]}}}`,
		},
		{
			m:    Message{Type: 3, Games: []game.Info{{ID: 7, Status: 2, Players: []string{"fred", "barney"}, CreatedAt: 1257894000}}},
			want: `{"type":3,"games":[{"id":7,"status":2,"players":["fred","barney"],"createdAt":1257894000}]}`,
		},
		{
			m:    Message{Type: 11},
			want: `{"type":11}`,
		},
		{
			...
			want: `{"type":1,"game":{"board":{"config":{"r":23,"c":21}},"config":{"checkOnSnag":true,"penalize":true,"minLength":3}}}`,
		},
	}
	for i, test := range MessageJSONTests {
		got, err := json.Marshal(test.m)
		switch {
		case err != nil:
		...
		var m2 Message
		err = json.Unmarshal([]byte(test.want), &m2)
		switch {
		case err != nil:
			t.Errorf("Test %v (Unmarshal): unwanted error while unmarshalling json '%v': %v", i, test.want, err)
...
func TestMessageMarshalOmitsInternals(t *testing.T) {
	...
	case want != string(got):
		t.Errorf("wanted %v, got %s", want, got)
	}
}
