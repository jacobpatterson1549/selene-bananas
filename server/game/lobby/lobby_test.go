...
func TestRemoveUser(t *testing.T) {
	username := "barney"
	l := Lobby{
		socketMessages: make(chan message.Message, 1),
	}
	l.RemoveUser(username)
	m := <-l.socketMessages
	switch {
	case m.Type != message.PlayerDelete:
		t.Errorf("wanted player delete messageType (%v), got %v", message.PlayerDelete, m.Type)
...
func TestCreateGame(t *testing.T) {
	...
	l.createGame(ctx, m)
	switch {
	case l.gameCfg.Config != game.Config{}:
