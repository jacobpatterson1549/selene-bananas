...
func TestRunnerHandleLobbyMessage(t *testing.T) {
	handleLobbyMessageTests := []struct {
		...
		{ // game delete gets sent as a leave
			...
			wantOk:          true,
			wantPlayerGames: make(map[player.Name]map[game.ID]message.Addr),
		},
...
func TestRunnerHandleSocketMessage(t *testing.T) {
	handleSocketMessageTests := []struct {
		...
		{ // socket close
			...
			wantPlayerSockets: make(map[player.Name]map[message.Addr]chan<- message.Message),
			wantPlayerGames:   make(map[player.Name]map[game.ID]message.Addr),
			wantOk:            true,
			skipOutSend:       true,
		},
